 ...
 */
 ...
 ---- Fix that's causing sceen frm 3d coordinates to be inaccurate
 **********************************************************************/
...
void draw()
{
  ...
  if(users.size() > 0){
    ...
    float x = map(diff.x,-200,200,xMinMax[0],xMinMax[1]); //NEEDS WORK
    x = x > xMinMax[1] ? xMinMax[1] : x;
    x = x < xMinMax[0] ? xMinMax[0] : x;
    
    float z = map(diff.z,-200,200,zMinMax[0],zMinMax[1]); //NEEDS WORK
    z = z > zMinMax[1] ? zMinMax[1] : z;
    z = z < zMinMax[0] ? zMinMax[0] : z;
    myMessage.add(x);
    myMessage.add(z);
    //println(myMessage);
